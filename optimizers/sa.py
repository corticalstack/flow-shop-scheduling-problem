...
class SA(Optimizer):
    ...
    def anneal(self):
        ...
        while self.prb.budget['remaining'] > 0 and (self.temp > self.temp_threshold):
            ...
            loss = self.best.fitness - new.fitness
            probability = math.exp(loss / self.temp)

            rr = random.random()
            if (new.fitness < self.best.fitness) or (rr < probability):
                lg.msg(logging.DEBUG, 'Previous best is {}, now updated with new best {}'.format(self.best.fitness,
                                                                                                 new.fitness))
                if rr < probability:
                    lg.msg(logging.DEBUG, 'Random {} less than probability {}'.format(rr, probability))
                self.best.fitness = new.fitness
                self.best.perm = new.perm
    ...
    def neighbour_solution(self):
        ...
        new_candidate_perm = self.best.perm.copy()
